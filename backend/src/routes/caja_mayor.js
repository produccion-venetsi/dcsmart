...
//
// Solo super_admin, y a diferencia del resto de gestión NO usa appContext: se ve
// todos los locales de todos los grupos a la vez, que es como se usa la app vieja
// (la maneja una persona en DC que filtra por local desde adentro). Por eso el
// guard es authenticate + requireSuperAdmin, sin X-App-Id.
//
...
function whereMovimientos({ id_local, id_app, moneda, estado, desde, hasta, origen }) {
...
export default async function cajaMayorRoutes(fastify) {
  ...
  fastify.get('/', { preHandler: guard }, async (request, reply) => {
    ...
    const filtros = { id_local, id_app, moneda, estado, desde, hasta, origen }
  ...
  fastify.get('/saldos', { preHandler: guard }, async (request, reply) => {
    const { moneda, desde, hasta, id_app, id_local } = request.query
    if (moneda && !MONEDAS.includes(moneda)) {
      return reply.code(400).send({ error: `moneda debe ser una de: ${MONEDAS.join(', ')}` })
    }
    const where = whereMovimientos({ moneda, desde, hasta, id_app, id_local })
  ...
  fastify.put('/estado', { preHandler: guard }, async (request, reply) => {
    ...
    if (id) {
      const existe = await fastify.db.movimientoCM.findUnique({ where: { id }, select: { id: true } })
  ...
  fastify.get('/locales', { preHandler: guard }, async (request, reply) => {
